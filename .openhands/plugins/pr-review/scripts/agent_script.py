...
def _noop_token_callback(_chunk: Any) -> None:
    """Accept streamed LLM chunks when no live token output is needed."""
...
def validate_environment() -> dict[str, Any]:
    ...
    if agent_kind == "openhands" and not api_key:
        logger.error("LLM_API_KEY is required when AGENT_KIND is 'openhands'")
        sys.exit(1)
    ...
    return {
        ...
        "stream": _get_bool_env("LLM_STREAM"),
...
def create_conversation(
    ...
    if config.get("stream"):
        conversation_kwargs["token_callbacks"] = [_noop_token_callback]
    if use_sub_agents:
